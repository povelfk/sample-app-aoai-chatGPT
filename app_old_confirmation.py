...
class ConversationHandler:
    ...
    def conversation_internal(self, request_body):
        # check token count, if token count over max token count, return hard-coded response
        # set max token count
        # count current tokens in request_body
        # if token count over max token count, return hard-coded response

        # Initialize session variables if not already present
        if 'previous_function' not in session:
            session['previous_function'] = None
        if 'user_confirmed' not in session:
        ...
        if CURRENT_TOKEN_COUNT > 50000:
            return generate_error_message("Du har nått maxgränsen av antal ord, vänligen starta en ny konversation.", request_body)
        ...
        if isinstance(classification_result, str):  # Check if the result is a string (error message)
            return generate_error_message(classification_result, request_body)
        else:
            function_to_call, parameters, is_function, classification_message = classification_result
            if is_function:
                if function_to_call != session['previous_function']:
                    session['previous_function'] = function_to_call
                    session['user_confirmed'] = False
                if self.function_requires_confirmation.get(session['previous_function'], False) and not session['user_confirmed']:
                    user_confirmation, follow_up_question, status_code = ask_confirmation_question(parameters, classification_message, function_to_call, request_body)
                    if user_confirmation.lower() != "ja":
...
def get_conversation():
    session['user_confirmed'] = False # reset user confirmation on page refresh
    authenticated_user = get_authenticated_user_details(request_headers=request.headers)
    user_id = authenticated_user['user_principal_id']
